...
function Add({data,type,refeshFunction}) {  
  ...
  return (
        <form onSubmit={handleSubmit(onSubmit)} className='grid grid-cols-4 grid-rows-4'>
          <InputDefault
          ...
          />
          <InputDefault
            defaultInput={type == "update" ? new Date (data.LaterUpdate).toISOString().split('T')[0] : ""}
            label="LaterUpdate"
          ...
          <InputFileDefault
            ...
            type="file"
            validate={""}
            register={(name, options) => register(name, { ...options, onChange: handleImageChange })}
            multiple={true}
            error={errors}
          />
          <div className='col-start-1 col-end-5 flex gap-2'>          
          {file.length > 0 && file.map((item, index) => (
            <img key={index} src={item} altInput={`preview-${index}`} style={{ width: '5%', maxHeight: 'auto', objectFit: 'cover' }} />
          ))}
          </div>
          {loading && <p>Loading...</p>}
          <button type="submit" className="col-start-1 col-end-5 mt-4 bg-blue-500 text-white py-2 px-4 w-full rounded hover:bg-blue-600">
